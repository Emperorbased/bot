...
TOKEN = "8546823835:AAFI-3t1SCB9S4PI5izbAAz1XEwHjRlL-6E"
SUPER_ADMINS = {7355737254, 8243127223, 8167127645}
admins = SUPER_ADMINS.copy()

WAITING_APPEAL, WAITING_COMPLAINT, WAITING_ADMIN_ID, WAITING_RESPONSE, WAITING_BAN_DURATION, WAITING_BAN_REASON, WAITING_BROADCAST, WAITING_BN_ID, WAITING_BN_DURATION, WAITING_BN_REASON = range(10)
...
async def gov(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"📢 Рассылка\n\n"
        f"Напишите текст для отправки всем пользователям:\n"
...
async def receive_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📤 Отправка началась...")
...
async def bn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👤 Отправьте ID пользователя для бана:")
    return WAITING_BN_ID

async def receive_bn_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Получение ID для бана"""
    try:
        target_id = int(update.message.text)
        context.user_data['bn_target'] = target_id
        
        await update.message.reply_text(
            f"⏱ Введите время бана для ID {target_id}:\n\n"
            f"Примеры: 1m, 5m, 1h, 12h, 1d, 7d"
        )
...
async def receive_bn_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.pop('bn_target', None)
    context.user_data.pop('bn_duration', None)
    context.user_data.pop('bn_duration_readable', None)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "appeal":
        ...
        if user_id in active_chats:
            await query.edit_message_text(
                "💬 У вас уже есть активный чат с администратором.\n"
        ...
        await query.edit_message_text(
            "✅ Запрос на чат отправлен администраторам.\n"
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=f"💬 Администратор @{query.from_user.username or query.from_user.first_name} подключился к чату!\n\n"
                     f"Все ваши сообщения будут переслаты администратору.",
                reply_markup=reply_markup
            )
        except Exception as e:
            logger.error(f"Ошибка: {e}")
        ...
        logger.info(f"Создан чат: {chat_user_id} (@{username}) <-> админ {admin_id}")
        return
        ...
        if appeal_id in appeals:
            ...
            await query.edit_message_text(
                f"{query.message.text}\n\n"
                f"🔒 Жалоба закрыта администратором"
            )
        return ConversationHandler.END
...
button_handler, pattern="^respond_")],
        states={
    ...
    bn_handler = ConversationHandler(
        entry_points=[CommandHandler("bn", bn)],
        states={
            WAITING_BN_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_bn_id)],
            WAITING_BN_DURATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_bn_duration)],
            WAITING_BN_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_bn_reason)],
        },
        fallbacks=[CommandHandler("cancel", cancel)],
    )
